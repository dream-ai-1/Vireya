...
export default function ForecastDrawer({ isOpen, onClose, forecastList }: DrawerProps) {
  ...
  return (
    <>
      ...
      <div className={`fixed top-0 left-0 h-screen w-[400px] bg-black/90 backdrop-blur-2xl z-[50] border-r border-white/10 transition-transform duration-500 p-10 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex justify-between items-center mb-10">
          <h2 className="text-2xl font-light text-white">5-Day Forecast</h2>
          ...
          {dailyData.map((day: any, i: number) => (
            <div key={i} className="bg-white/5 p-5 rounded-2xl border border-white/5">
              ...
              <div className="flex justify-between items-center">
                <span className="text-xl font-medium text-white">{Math.round(day.main.temp)}°</span>
                <span className="text-xs opacity-70 text-white capitalize">{day.weather[0].description}</span>
              </div>
            </div>
          ))}
        </div>
